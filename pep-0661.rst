...
with itself.  Similarly, checking using ``if value is MISSING:`` should
usually be preferred over checks such as ``if value:`` or ``if not value:``.
...
[#reference-github-repo]_.  A simplified version follows::
    ...
    def sentinel(name, repr=None):
        ...
        # Make the object a singleton, by having the class always return the
        # same object.
